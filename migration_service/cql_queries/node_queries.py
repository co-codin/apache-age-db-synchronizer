...
alter_nodes_query_create_fields = "WITH $nodes as node_batch " \
                                  "UNWIND node_batch as node_record " \
                                  "MATCH (node {db: $db_source + '.' + node_record.name}) " \
                                  "WITH node_record.fields_to_create as fields_to_create, node " \
                                  "UNWIND fields_to_create as field " \
                                  "CREATE (node)-[:ATTR]->(:Field {name: field.name, db: node.db + '.' + field.name, attrs: [], dbtype: field.db_type});"

alter_nodes_query_delete_fields = "WITH $nodes as node_batch " \
                                  "UNWIND node_batch as node_record " \
                                  "MATCH (node {db: $db_source + '.' + node_record.name}) " \
                                  "WITH node_record.fields_to_delete as fields_to_delete, node " \
                                  "UNWIND fields_to_delete as field " \
                                  "MATCH (node)-[:ATTR]->(f:Field {db: node.db + '.' + field}) " \
                                  "DETACH DELETE f;"


alter_nodes_query_alter_fields = "WITH $nodes as node_batch " \
                                 "UNWIND node_batch as node_record " \
                                 "MATCH (node {db: $db_source + '.' + node_record.name}) " \
                                 "WITH node_record.fields_to_alter as fields_to_alter, node " \
                                 "UNWIND fields_to_alter as field " \
                                 "MATCH (node)-[:ATTR]->(f:Field {db: node.db + '.' + field.name}) " \

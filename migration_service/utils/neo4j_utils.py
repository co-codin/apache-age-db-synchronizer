...
async def get_neo4j_tables(graph_session: Neo4jAsyncSession) -> Set[str]:
    res = await graph_session.run(
        ...
        db_source=settings.db_source
    )
    table_names = await res.value('name')
    return set(table_names)


async def get_neo4j_table_col_type(table_names: Set[str], graph_session: Neo4jAsyncSession):
    res = await graph_session.run(
        ...
        "ORDER BY db_name;",
        db_source=settings.db_source, tables=list(table_names)
    )
